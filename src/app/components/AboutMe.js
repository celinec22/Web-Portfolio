export default function AboutMe() {
  return (
    <div className="overflow-hidden rounded-sm mt-10 ">
      <h1 id="AboutMe" className="text-xl font-bold mb-2">
        About Me
      </h1>
      <hr className="border-gray-600 mb-4" />

      <p className="text-gray-300 text-base/8 font-bold">
        Hi! My name is Celine. I’m a student at the University of Michigan -
        Dearborn, majoring in Software Engineering & CIS Math. I’ve always been
        passionate about building and problem-solving, and development has given
        me the perfect outlet to do both.
      </p>
      <hr className="mt-8 border-gray-600" />
    </div>
  );
}
